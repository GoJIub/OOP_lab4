...
TEST(TrapezoidTest, DegenerateCase) {
    Trapezoid<double> t;
...
// --- ТЕСТЫ ДЛЯ Square ---
TEST(SquareTest, AreaAndValidate) {
    Square<double> s;
...
TEST(RectangleTest, DegenerateThrows) {
    Rectangle<double> r;
    EXPECT_THROW(inputFigure(r, "1 1  1 1  1 1  1 1"), std::invalid_argument);
}
...
TEST(ArrayTest, TotalSurface) {
    ...
    inputFigure(*s, "0 0  2 0  2 2  0 2"); // площадь 4
    arr.add(s);
